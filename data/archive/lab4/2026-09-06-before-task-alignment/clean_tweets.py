"""Clean airline tweets, calculate TF-IDF, and estimate sentiment with RoBERTa.
...
def ensure_wordnet() -> None:
    """Download the WordNet corpus only when it is not already available."""
    try:
...
def add_tfidf_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    ...
    def preprocess(text: str) -> str:
        tokens = tokenizer.tokenize(normalize_for_tfidf(text))
        lemmas = [
    ...
    df = df.copy()
    df["text_clean"] = df["tweet_text_raw"].apply(preprocess)
